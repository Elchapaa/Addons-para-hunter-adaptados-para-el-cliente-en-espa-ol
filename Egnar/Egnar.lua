...
local WingClip = 6 --Cortal alas 
local AutoShot = 2 -- Disparo automatico

function Egnar_OnLoad()
  Egnar_Frame:Hide()
  _, cl = UnitClass("player")
  if cl ~= "HUNTER" or cl~= "CAZADOR" then
  ...
  if IsActionInRange(WingClip) == 1 then
    ...
    elseif IsActionInRange(AutoShot) == 1 then
      if CheckInteractDistance("target", 4) then
        FontString1:SetText("En rango")
        SetColor(unpack({0, 0.5, 1, 0.7}))
      else
        FontString1:SetText("Largo alcance")
        SetColor(unpack({0, 0, 1, 0.7}))
      end
    elseif CheckInteractDistance("target", 4) then
      FontString1:SetText("Zona muerta")
      SetColor(unpack({1, 0.5, 0, 0.7}))
    else
      FontString1:SetText("Fuera de rango")
